#!/usr/bin/env python3
"""Run the frozen eight-family CIB confirmatory suite without reusing outputs."""
...
CIB_RELEASE_COMMIT = "84126513f487df605b45f8bb4286246ba1801e5a"
...
EXPECTED_CIB_VERSION = "cib 0.5.0"
...
def main() -> int:
    ...
    parser.add_argument("--parallel-studies", type=int, default=4)
    parser.add_argument(
    ...
    if not 1 <= args.parallel_studies <= 4:
        parser.error("--parallel-studies must be between 1 and 4")
    ...
    if cib_commit != CIB_RELEASE_COMMIT:
        parser.error(
            f"CIB checkout must be the frozen v0.5.0 commit {CIB_RELEASE_COMMIT}"
        )
    ...
    protocol_check = subprocess.run(
        [sys.executable, str(ROOT / "scripts" / "validate_confirmatory_protocol.py")],
        cwd=ROOT,
        capture_output=True,
        text=True,
        check=False,
    ...
    index: dict[str, object] = {
        "schema_version": "cib-confirmatory-run-index/1",
        "state": "running",
