#!/usr/bin/env python3
"""Analyze the frozen 64-pair CIB corpus without publishing private evidence."""
...
FROZEN_CIB_COMMIT = "84126513f487df605b45f8bb4286246ba1801e5a"
REQUIRED_ARTIFACTS = (
    "run-manifest.jsonl",
...
def _validate_run_index(
    run_index: dict[str, Any], lock: dict[str, Any]
) -> tuple[dict[str, dict[str, Any]], dict[str, str]]:
    if run_index.get("schema_version") != "cib-confirmatory-run-index/1":
...
def _load_cells(run_root: Path) -> tuple[dict[CellKey, Cell], list[dict[str, Any]]]:
    lock = json.loads((STUDY_ROOT / "protocol-lock.json").read_text())
    run_index = json.loads((run_root / "run-index.json").read_text())
    indexed, planned_hashes = _validate_run_index(run_index, lock)
    cells: dict[CellKey, Cell] = {}
    family_rows: list[dict[str, Any]] = []
    for family in FAMILIES:
        config = STUDY_ROOT / "configs" / f"{family}.yaml"
        ...
        if provenance.get("generator_version") != "0.5.0":
            raise ValueError(f"{family} report was not generated by CIB v0.5.0")
...
def analyze(
    ...
    *,
    ...
    return {
        "schema_version": "cib-confirmatory-summary/1",
...
def main() -> int:
    ...
    cells, family_rows = _load_cells(args.run_root.resolve())
